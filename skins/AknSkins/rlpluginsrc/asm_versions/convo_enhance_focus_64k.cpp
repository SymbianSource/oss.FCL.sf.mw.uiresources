...
void AConvolute64KEnhanceFocus( CFbsBitmap& aTarget,
    ...
    ADoConvolute64KEnhanceFocus(targetAddr, sourceAddr, combinedScanW, width, height, aBlendFactor);
...
#ifndef ARM_VERSION
__NAKED__ void ADoConvolute64KEnhanceFocus( TUint*, TUint*, TInt, TInt, TInt, TInt )
    {
//    return;
#else
__NAKED__ void ADoConvolute64KEnhanceFocus( TUint* aBaseT,
                                            TUint* aBaseS,
                                            TInt aCombinedScanW,
                                            TInt aWidth,
                                            TInt aHeight,
...
widthloop
    ...
    MOV r11, #0x1f  
    MOV r11, r11, LSL #19    // for filtering red, r11 used in blending
    ...
    ADD r1, r1, r2           // we skipped middle row, so add r2
    ...
    ADD r8, r8, r4           // r8 = a1 + a7 + a9 + a3
    AND r4, r12, r7, LSR #16 // r4 = blue-value of b3
    AND r5, r10, r7, LSR #12
    ORR r4, r4, r5           // r4 = green and blue of b3
    AND r5, r11, r7, LSR #8 
    ORR r4, r4, r5           // r4 = red, green and blue of b3
    ADD r9, r9, r4           // r9 = b1 + b7 + b9 + b3



    // now r8 has pixels a2, a4, a6 and a8
    // convolution sum would be a5 * 9 - (a2 + a4 + a6 + a8)
    // free registers to use are r3, r4, r5, r7, r11 and r12
    ...
    MOV r12, r12, lsr #1        
    ADD r12, r12, #1             // divided by 3 (adding 1 to get more close result)
    ...
    ORR r4, r4, r12, LSL #5  // r4 now has both blue and green
    ...
    ORR r4, r4, r12, LSL #11 // r4 now has red, green and blue
