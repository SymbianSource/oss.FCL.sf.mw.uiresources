...
* Contributors:
*
* Description:   ELOCL.DLL for Denmark, ELOCL.07
*
*/
...
const TLanguage LLocaleData::Language = ELangDanish;
...
const TText* const LLocaleData::ShortDateFormatSpec = _S("%F%/0%D%/1%M%/2%*Y%/3"); // year.month,day (leading zeros, year with 2 digits)
const TText* const LLocaleData::LongDateFormatSpec = _S("%F%*D. %N %Y"); // day month(name) year (no leading zero in day)
const TText* const LLocaleData::TimeFormatSpec = _S("%F%:0%H%:1%T%:3%S%:3"); // hours:minutes:seconds (with leading zeros)
