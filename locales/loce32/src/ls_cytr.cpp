...
* Contributors:
*
* Description:   ELOCL.DLL for Turkey, ELOCL.14
*
*/
...
const TLanguage LLocaleData::Language = ELangTurkish;	
const TInt LLocaleData::CountryCode = 90;   
const TInt LLocaleData::UniversalTimeOffset = 7200;
const TDateFormat LLocaleData::DateFormat = EDateEuropean; // dd/mm/yyyy
...
const TText * const LLocaleData::CurrencySymbol = _S("TL");
const TText* const LLocaleData::ShortDateFormatSpec = _S("%F%/0%D%/1%*M%/2%Y%/3"); // day.month.year (with leading zeros) 
const TText* const LLocaleData::LongDateFormatSpec = _S("%F%*D %N %Y"); // day month(name) year (no leading zero in day)
const TText* const LLocaleData::TimeFormatSpec = _S("%F%:0%J%:1%T%:2%S%:3%B"); // hours:minutes:seconds (with leading zeros)
