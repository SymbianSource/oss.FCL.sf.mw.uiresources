...
* Contributors:
*
* Description:   ELOCL.DLL for Brazilian Portuguese
*
*/
...
const TLanguage LLocaleData::Language = ELangBrazilianPortuguese;
const TInt LLocaleData::CountryCode = 55;
const TInt LLocaleData::UniversalTimeOffset = -10800;
...
const TText * const LLocaleData::CurrencySymbol = _S("R$");
const TText* const LLocaleData::ShortDateFormatSpec = _S("%F%/0%*D%/1%*M%/2%Y%/3"); // day/month/year (no leading zeros)
const TText* const LLocaleData::LongDateFormatSpec = _S("%F%*D de %N de %Y"); // day de month(name) de year (no leading zero in day)
const TText* const LLocaleData::TimeFormatSpec = _S("%F%:0%*H%:1%*T%:2%*S%:3"); // hours:minutes:seconds (without leading zeros)
