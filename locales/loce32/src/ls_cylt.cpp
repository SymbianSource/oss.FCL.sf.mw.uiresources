...
* Contributors:
*
* Description:   ELOCL.DLL for Lithuania, ELOCL.68
*
*/
...
const TLanguage LLocaleData::Language = ELangLithuanian;
const TInt LLocaleData::CountryCode = 370;
...
const TText * const LLocaleData::TimeSeparator[KMaxTimeSeparators] = {_S(""),_S(":"),_S(":"),_S("")}; // seconds separator???
...
const TText * const LLocaleData::CurrencySymbol = _S("Lt");
const TText* const LLocaleData::ShortDateFormatSpec = _S("%F%/0%Y%/1%M%/2%D%/3"); // day.month.year (without leading zeros)
const TText* const LLocaleData::LongDateFormatSpec = _S("%F%Y m. %N %*D d."); // day(dot separator) month(name) year
const TText* const LLocaleData::TimeFormatSpec = _S("%F%:0%*H%:1%T%:2%S%:3"); // hours.minutes.seconds (with leading zeros) !!!seconds separator!!!
