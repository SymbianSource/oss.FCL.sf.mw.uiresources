...
* Contributors:
*
* Description:   ELOCL.DLL for Italy, ELOCL.05
*
*/
...
const TText* const LLocaleData::ShortDateFormatSpec = _S("%F%/0%D%/1%M%/2%Y%/3"); // (leading zeros)
const TText* const LLocaleData::LongDateFormatSpec = _S("%F%D %N %Y"); // (no leading zero in day)
const TText* const LLocaleData::TimeFormatSpec = _S("%F%:0%*H%:1%*T%:2%*S%:3"); // (with leading zeros)
