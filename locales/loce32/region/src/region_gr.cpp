...
* Contributors:
*
* Description:   elocl_reg.300 for Greece region
*
*/
...
const TInt RegionAspect::CountryCode = 30;
...
const TText* const RegionAspect::ShortDateFormatSpec = _S("%F%/0%*D%/1%*M%/2%Y%/3"); // no leading zeros 4-digit year
const TText* const RegionAspect::LongDateFormatSpec = _S("%F%*D %N %Y"); // 7. monthname 2001
const TText* const RegionAspect::TimeFormatSpec = _S("%F%:0%I%:1%T%:2%S%:3"); // 02.01.01
...
const TUint16 RegionAspect::RegionCode = 300;
