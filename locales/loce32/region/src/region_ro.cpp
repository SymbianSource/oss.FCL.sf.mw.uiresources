...
* Contributors:
*
* Description:   elocl_reg.642 for Romania region
*
*/
...
const TInt RegionAspect::CountryCode = 40;
...
const TText* const RegionAspect::CurrencySymbol = _S("RON");
const TText* const RegionAspect::ShortDateFormatSpec = _S("%F%/0%D%/1%M%/2%Y%/3"); // day.month.year (leading zeros)
const TText* const RegionAspect::LongDateFormatSpec = _S("%F%*D %N %Y"); // day month(name) year (leading zero in day)
...
const TUint16 RegionAspect::RegionCode = 642;
