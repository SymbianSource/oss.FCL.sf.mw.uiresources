...
* Contributors:
*
* Description:   elocl_reg.428 for Latvia region
*
*/
...
const TInt RegionAspect::CountryCode = 371;
...
const TText * const RegionAspect::CurrencySymbol = _S("Ls");
const TText* const RegionAspect::ShortDateFormatSpec = _S("%F%/0%*D%/1%*M%/2%Y%/3"); // day.month.year (with leading zeros)
const TText* const RegionAspect::LongDateFormatSpec = _S("%F%Y. gada %*D. %N"); // day(dot separator) month(name) year
...
const TUint16 RegionAspect::RegionCode = 428;
