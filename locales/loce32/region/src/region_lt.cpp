...
* Contributors:
*
* Description:   elocl_reg.440 for Lithuania region
*
*/
...
const TInt RegionAspect::CountryCode = 370;
...
const TText * const RegionAspect::TimeSeparator[KMaxTimeSeparators] = {_S(""),_S(":"),_S(":"),_S("")}; // seconds separator???
...
const TText * const RegionAspect::CurrencySymbol = _S("Lt");
const TText* const RegionAspect::ShortDateFormatSpec = _S("%F%/0%Y%/1%M%/2%D%/3"); // day.month.year (without leading zeros)
const TText* const RegionAspect::LongDateFormatSpec = _S("%F%Y m. %N %*D d."); // day(dot separator) month(name) year
const TText* const RegionAspect::TimeFormatSpec = _S("%F%:0%*H%:1%T%:2%S%:3"); // hours.minutes.seconds (with leading zeros) !!!seconds separator!!!
...
const TUint16 RegionAspect::RegionCode = 440;
