...
* Contributors:
*
* Description:   elocl_reg.208 for Denmark region
*
*/
...
const TInt RegionAspect::CountryCode = 45;
...
const TText* const RegionAspect::ShortDateFormatSpec = _S("%F%/0%D%/1%M%/2%*Y%/3"); // year.month,day (leading zeros, year with 2 digits)
const TText* const RegionAspect::LongDateFormatSpec = _S("%F%*D. %N %Y"); // day month(name) year (no leading zero in day)
const TText* const RegionAspect::TimeFormatSpec = _S("%F%:0%H%:1%T%:3%S%:3"); // hours:minutes:seconds (with leading zeros)
...
const TUint16 RegionAspect::RegionCode = 208;	
